...
#This predictive model uses the growth curve 4Y4 models proposed by Yu
...
# Define the equation
def equation_fw(t, a1, b1, c1):
    return ( (np.exp(a1*t**b1)- 1)/((np.exp(a1*t**b1)) + 1))**c1
...
initial_guess1 = [1, 1, 1]  # You can modify these values as per your requirement
...
for method in methods1:
    try:
        ...
        a1_fit, b1_fit, c1_fit = constants1
        ...
        fw_result = equation_fw(t_range, a1_fit, b1_fit, c1_fit)
        ...
        for x in range(0, len(forecasted_df)):
            ...
            WC_f = equation_fw(time, a1_fit, b1_fit, c1_fit)
...
# Define the equation
def equation_Np(t, Nrmax, a2, b2, c2):
    return Nrmax * ((np.exp(a2 * t**b2) - 1) / (np.exp(a2 * t**b2) + 1))**c2
...
# Define the initial guesses for the constants
initial_guess2 = [Np_last, 1, 1, 1]  # You can modify these values as per your requirement

# Define the bounds
bounds = ([Np_last, 0, 0, 0], [OOIP, 100, 10, np.inf])
...
for method in methods2:
    try:
        ...
        Nrmax_fit, a2_fit, b2_fit, c2_fit = constants2
        ...
        # Plot the equation curve
        Np_result = equation_Np(t, Nrmax_fit, a2_fit, b2_fit, c2_fit)
        ...
        for i in range(0, len(forecasted_df)):
            ...
            Np_f = equation_Np(time, Nrmax_fit, a2_fit, b2_fit, c2_fit)
...
Np_actual = forecasted_df['Np (MMSTB)']
...
# Plot the equation curve for water cut
Np_result = equation_Np(t_range, Nrmax_fit, a2_fit, b2_fit, c2_fit)
...
galat_WC = [value for value in galat_WC if not pd.isna(value)]
...
r2_Np = r2_score(Np_actual, Np_forecast)
